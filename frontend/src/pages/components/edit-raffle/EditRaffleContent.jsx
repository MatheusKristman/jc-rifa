...
const EditRaffleContent = () => {
    ...
    useEffect(() => {
        const fetchRaffleParticipants = () => {
            if (raffleSelected.hasOwnProperty("_id") && participants.length === 0) {
                setToLoadUsersNumberBuyed();
                api.post("/edit-raffle/get-users", {
    ...
    useEffect(() => {
        function fetchWinner() {
            if (!winner.hasOwnProperty("_id") && raffleSelected.hasOwnProperty("_id")) {
                setToFetchWinner();
                api.post("/edit-raffle/winner", { title: raffleSelected.title })
                    .then((res) => {
                        setWinner(res.data);
                        setNotToFetchWinner();
                    })
        ...
        const submitData = () => {
            if (isSubmitting) {
                const sendDataToDB = () => {
                    ...
                    api.put("/edit-raffle/updating", formData, {
                        headers: {
    ...
    const resetWinner = () => {
        api.delete(`/edit-raffle/cancel/${winner._id}`)
            .then((res) => {
                if (res.data) {
                    setWinner({});
                    setFinishNumberFromFetch("");
