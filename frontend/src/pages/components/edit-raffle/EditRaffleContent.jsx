...
// TODO arrumar a forma que os dados são recebidos para esse novo formato
const EditRaffleContent = () => {
    const {
        ...
        (state) => ({
            ...
            activateDeleteConfirmationAnimation: state.activateDeleteConfirmationAnimation,
    ...
    const { setToLoad, setNotToLoad, setToAnimateFadeIn, setToAnimateFadeOut, isLoading } =
    ...
    const finishRaffle = () => {
        if (finishNumber) {
            setToChooseWinner();
            api.post("/raffle/generate-a-winner", {
                ...
                .catch((error) => {
                    ...
                    if (error?.response.data === "Número não foi comprado, insira outro número") {
                        ...
                        error?.response.data === "Usuário não encontrado, insira um novo número"
                    ) {
                        setFinishNumberError(error.response.data);
                        setWinner({});
                    } else {
    ...
    const resetWinner = () => {
        api.delete(`/winner/delete-winner/${winner._id}`)
            .then((res) => {
                if (res.data) {
                    setWinner({});
                    setFinishNumberFromFetch("");
                ...
                <div className="edit-raffle__content__container__finish-raffle-container">
                    ...
                    {winner.hasOwnProperty("_id") ? null : (
                        <input
                            ...
                            style={
                                finishNumberError ? { border: "2px solid rgb(209, 52, 52)" } : {}
                            }
