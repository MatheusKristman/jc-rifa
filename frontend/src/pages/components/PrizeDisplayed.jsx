...
const PrizeDisplayed = ({ image, title, subtitle, progress }) => {
  ...
  return (
    <div className="prize-displayed prize-clickable">
      <div className={isRaffleLoading ? "prize-displayed__image-box loading" : "prize-displayed__image-box"}>
        <img src={image?.data ? `data:${image.contentType};base64,${_arrayBufferToBase64(image.data.data)}` : defaultPrize} alt="Prêmio" className="prize-displayed__image-box__image" />
      </div>

      <div className="prize-displayed__infos-box">
        <h3 className={isRaffleLoading ? "prize-displayed__infos-box__title loading" : "prize-displayed__infos-box__title"}>
          {title ? title : 'Título Carregando'}
        </h3>

        <span className={isRaffleLoading ? "prize-displayed__infos-box__desc loading" : "prize-displayed__infos-box__desc"}>
